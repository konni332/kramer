// PeSTO tables — from white's perspective, a8=index 0, h1=index 63
// For black we mirror vertically using FLIP (sq ^ 56)
// Material is baked into the tables: table[sq] = material_value + pst_bonus
...
pub static MG_TABLE: [[i32; 64]; 13] = build_mg_table();
pub static EG_TABLE: [[i32; 64]; 13] = build_eg_table();
...
const fn build_mg_table() -> [[i32; 64]; 13] {
    let mut table = [[0i32; 64]; 13];
    let mut p = 0usize;
    while p < 6 {
        let mut sq = 0usize;
        while sq < 64 {
            table[p][sq] = MG_VALUE[p] + MG_PST[p][sq ^ 56];
            table[p + 6][sq] = MG_VALUE[p] + MG_PST[p][sq];
            sq += 1;
        }
...
const fn build_eg_table() -> [[i32; 64]; 13] {
    let mut table = [[0i32; 64]; 13];
    let mut p = 0usize;
    while p < 6 {
        let mut sq = 0usize;
        while sq < 64 {
            table[p][sq] = EG_VALUE[p] + EG_PST[p][sq ^ 56];
            table[p + 6][sq] = EG_VALUE[p] + EG_PST[p][sq];
            sq += 1;
        }
