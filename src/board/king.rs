...
impl Board {
    pub fn generate_king_moves(&self, list: &mut MoveList) {
        ...
        while bb != 0 {
            ...
            self.push_king_quiets(from, quiet, piece, list);
            self.push_king_caps(from, caps, piece, list);
        }
    }

    fn push_king_quiets(&self, from: u8, mut bb: Bitboard, piece: u8, list: &mut MoveList) {
        while bb != 0 {
    ...
    fn push_king_caps(&self, from: u8, mut bb: Bitboard, piece: u8, list: &mut MoveList) {
        while bb != 0 {
            let to = bb.trailing_zeros() as u8;
            bb &= bb - 1;
